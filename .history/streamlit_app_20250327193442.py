...
def create_attention_graph(
    ...
    # Track which heads we've already added to the legend
    added_to_legend = set()
    ...
    for pattern in filtered_patterns:
        ...
        head_belongs_to_group = False
        group_name = None
        for group in head_groups:
            if any(h.layer == pattern['sourceLayer'] and h.head == pattern['head'] for h in group.heads):
                head_belongs_to_group = True
                group_name = group.name
                break

        # If head doesn't belong to a group and hasn't been added to legend yet
        if not head_belongs_to_group and (pattern['sourceLayer'], pattern['head']) not in added_to_legend:
            legend_name = f"Head {pattern['sourceLayer']},{pattern['head']}"
            added_to_legend.add((pattern['sourceLayer'], pattern['head']))
        else:
            legend_name = group_name if head_belongs_to_group else None
