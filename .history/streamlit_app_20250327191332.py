                    ...
                    st.markdown(f"""
                        <div class="group-container">
                            <div style="display: flex; align-items: center; gap: 8px;">
                                <div style="width: 20px; height: 20px; background-color: {group.color or COLOR_PALETTE[group.id % len(COLOR_PALETTE)]}; border-radius: 4px;"></div>
            ...
            if st.session_state.selected_heads:
                ...
                for head in st.session_state.selected_heads:
                    ...
                    with col1:
                        st.markdown(f"""
                            <div style="display: flex; align-items: center; gap: 4px;">
                                <div style="display: inline-block;">
                                    <div style="width: 20px; height: 20px; border-radius: 4px; cursor: pointer;">
                                        {st.color_picker(f"Color for {head.layer},{head.head}", value=head.color or '#3B82F6', key=f"color_{head.layer}_{head.head}", label_visibility="collapsed")}
                                    </div>
                                </div>
                        ...
                        """, unsafe_allow_html=True)
                        if st._get_last_color_picker_value() != head.color:
                            head.color = st._get_last_color_picker_value()
                            st.experimental_rerun()
                    with col2:
