...
def main():
    ...
    with st.expander("Controls", expanded=True):
        ...
        with col1:
            ...
            for group in st.session_state.head_groups:
                with st.container():
                    ...
                    """, unsafe_allow_html=True)
                    # Add color picker for the group
                    new_color = st.color_picker(
                        f"Color for {group.name}",
                        value=group.color or COLOR_PALETTE[group.id % len(COLOR_PALETTE)],
        ...
        with col2:
            ...
            # Display selected heads in a similar style to groups
            if st.session_state.selected_heads:
                st.markdown("""
                    <div class="group-container">
                        <div style="font-weight: 500;">Selected Heads</div>
                        <div style="display: flex; flex-wrap: wrap; gap: 4px;">
                """, unsafe_allow_html=True)
                
                for head in st.session_state.selected_heads:
                    st.markdown(f"""
                        <span class="head-button" style="background-color: #3B82F6">
                            {head.layer},{head.head}
                            <a href="#" onclick="removeHead({head.layer}, {head.head})" style="margin-left: 4px; color: white; text-decoration: none;">×</a>
                        </span>
                    """, unsafe_allow_html=True)
