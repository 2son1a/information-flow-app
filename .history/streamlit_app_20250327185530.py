...
def create_attention_graph(
    ...
    for pattern in filtered_patterns:
        ...
        head_type = None
        color = None  # Default to None
        
        # Check which group this head belongs to
        for idx, group in enumerate(head_groups):
            if any(h.layer == pattern['sourceLayer'] and h.head == pattern['head'] for h in group.heads):
                head_type = group.name
                color = COLOR_PALETTE[idx % len(COLOR_PALETTE)]  # Use group's color from palette
                break
        
        # If no group color was found, use default blue
        if color is None:
