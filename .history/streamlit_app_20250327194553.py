...
def create_attention_graph(
    ...
    # Then add individual head patterns
    for pattern in filtered_patterns:
        head_key = (pattern['sourceLayer'], pattern['head'])
        if head_key not in head_to_group:  # Only process heads not in any group
            x0, y0 = pattern['sourceToken'], pattern['sourceLayer']
            x1, y1 = pattern['destToken'], pattern['destLayer']
            ...
            # Find the corresponding selected head to get its color
            head_color = next(
                (h.color for h in selected_heads if h.layer == pattern['sourceLayer'] and h.head == pattern['head']),
                '#3B82F6'
            )
            ...
            fig.add_trace(go.Scatter(
                ...
                hovertemplate=f'Layer: {pattern["sourceLayer"]}<br>Head: {pattern["head"]}<br>Weight: {pattern["weight"]:.3f}',
                name=f"Head {pattern['sourceLayer']},{pattern['head']}",
                showlegend=True
            ))
