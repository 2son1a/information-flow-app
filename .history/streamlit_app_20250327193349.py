...
def create_attention_graph(
    ...
    # Add legend traces for individual heads that don't belong to any group
    used_colors = set(COLOR_PALETTE[idx % len(COLOR_PALETTE)] for idx in range(len(head_groups)))
    for head in selected_heads:
        # Check if this head belongs to any group
        belongs_to_group = any(
            any(h.layer == head.layer and h.head == head.head for h in group.heads)
            for group in head_groups
        )
        
        if not belongs_to_group:
            # Use the head's color or assign a new one
            color = head.color or next(
                (c for c in COLOR_PALETTE if c not in used_colors),
                COLOR_PALETTE[len(used_colors) % len(COLOR_PALETTE)]
            )
