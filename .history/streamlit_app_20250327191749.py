...
def main():
    ...
    with st.expander("Controls", expanded=True):
        ...
        with col1:
            ...
            if st.button("Create", type="primary"):
                if new_group:
                    new_group_obj = HeadGroup(
                        ...
                        name=new_group,
                        heads=[],
                        color=COLOR_PALETTE[len(st.session_state.head_groups) % len(COLOR_PALETTE)]  # Set default color
                    )
            ...
            # Single color picker for new groups
                    ...
                    st.markdown(f"""
                        <div class="group-container">
                            <div style="display: flex; align-items: center; gap: 8px;">
                                <div style="width: 20px; height: 20px; background-color: {group.color}; border-radius: 4px;"></div>
                                <div style="font-weight: 500;">{group.name}</div>
                            </div>
                            <div class="group-description">{group.description or ''}</div>
                            <div style="display: flex; flex-wrap: wrap; gap: 4px;">
                                {''.join([f'<span class="head-button" style="background-color: {group.color}">{head.layer},{head.head}</span>' for head in group.heads])}
                            </div>
                        </div>
                    """, unsafe_allow_html=True)
        ...
        with col2:
            ...
            # Single color picker for new heads
            new_head_color = st.color_picker(
                "",
                value='#3B82F6',
                key="new_head_color",
                label_visibility="collapsed"
                    ...
                    if layer_str == ':' and head_str == ':':
                        for l in range(st.session_state.attention_data['numLayers']):
                            for h in range(st.session_state.attention_data['numHeads']):
                                st.session_state.selected_heads.append(HeadPair(layer=l, head=h, color=new_head_color))
                    elif layer_str == ':':
                        layer = int(head_str)
                        for h in range(st.session_state.attention_data['numHeads']):
                            st.session_state.selected_heads.append(HeadPair(layer=layer, head=h, color=new_head_color))
                    elif head_str == ':':
                        head = int(layer_str)
                        for l in range(st.session_state.attention_data['numLayers']):
                            st.session_state.selected_heads.append(HeadPair(layer=l, head=head, color=new_head_color))
                    else:
                        layer = int(layer_str)
                        head = int(head_str)
                        st.session_state.selected_heads.append(HeadPair(layer=layer, head=head, color=new_head_color))
                    ...
                    with col1:
                        st.markdown(f"""
                            <div style="display: flex; align-items: center; gap: 4px;">
                                <span class="head-button" style="background-color: {head.color}">
