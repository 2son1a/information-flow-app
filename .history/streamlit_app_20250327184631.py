...
def create_attention_graph(
    ...
    for layer in range(data['numLayers']):
        for token in range(data['numTokens']):
            fig.add_trace(go.Scatter(
                ...
                mode='markers',
                marker=dict(
                    size=4,  # Smaller, more subtle nodes
                    color='#E5E7EB',  # Lighter grey for nodes
                ),
                hoverinfo='none',
                showlegend=False
            ))

    # Define colors for different head types
    head_type_colors = {
        'Subject Heads': '#10B981',  # emerald-500
        'Relation Heads': '#8B5CF6',  # violet-500
        'Mixed Heads': '#F59E0B',  # amber-500
    }

    # Add attention patterns as curved lines
    for pattern in filtered_patterns:
        # Determine the head type and color
        head_type = None
        color = '#3B82F6'  # Default blue
        
        # Check which group this head belongs to
        for group in head_groups:
            if any(h.layer == pattern['sourceLayer'] and h.head == pattern['head'] for h in group.heads):
                head_type = group.name
                if head_type in head_type_colors:
                    color = head_type_colors[head_type]
                break
        ...
        dx = abs(x1 - x0)
        if x0 == x1:  # Self-attention
            # Create vertical line with slight curve
            control_x = x0
            control_y = (y0 + y1) / 2
        else:
            # Adjust curve height based on token distance
            height_factor = min(0.5 * dx, 2.0)  # Cap the height factor
            control_x = (x0 + x1) / 2
            control_y = min(y0, y1) - height_factor
        
        # Generate points along the bezier curve
        t = np.linspace(0, 1, 50)  # More points for smoother curves
        x = (1-t)**2 * x0 + 2*(1-t)*t * control_x + t**2 * x1
        y = (1-t)**2 * y0 + 2*(1-t)*t * control_y + t**2 * y1
        
        # Base width on attention weight but keep it more subtle
        width = max(1, min(3, 1 + pattern['weight']))
    ...
    # Add legend traces for head types
    for head_type, color in head_type_colors.items():
        fig.add_trace(go.Scatter(
            x=[None],
            y=[None],
            mode='lines',
    ...
    fig.update_layout(
        ...
        xaxis=dict(
            ...
            showgrid=False,
            zeroline=False,
            tickfont=dict(size=10)  # Smaller font for tokens
        ),
        yaxis=dict(
            title='Layer',
            showgrid=False,
            ...
            tickfont=dict(size=10)  # Smaller font for layer numbers
        ),
        ...
        legend=dict(
            ...
            xanchor="right",
            x=0.99,
            font=dict(size=10)  # Smaller font in legend
        )
    )
...
def main():
    ...
    with col1:
        ...
        st.caption("Predefined head groups are specific to the selected model.")
    ...
    with st.expander("Controls", expanded=True):
                ...
                st.markdown(f"""
                    <div class="group-container">
                        ...
                        <div style="display: flex; flex-wrap: wrap; gap: 4px;">
                            {''.join([f'<span class="head-button" style="background-color: {COLOR_PALETTE[group.id % len(COLOR_PALETTE)]}">{head.layer},{head.head}</span>' for head in group.heads])}
                        </div>
                    </div>
                """, unsafe_allow_html=True)
        ...
        with col2:
            ...
            st.markdown("<div style='font-weight: 400; margin-bottom: 8px;'>Selected heads:</div>", unsafe_allow_html=True)
            
            if not st.session_state.selected_heads:
                st.markdown("<div style='color: #6b7280; font-style: italic;'>No heads selected</div>", unsafe_allow_html=True)
                st.markdown("<div style='color: #6b7280; font-size: 0.8rem; margin-top: 4px;'>↑ Click + to add a head to a group</div>", unsafe_allow_html=True)
                    ...
                    if layer_str == ':' and head_str == ':':
                        for l in range(st.session_state.attention_data['numLayers']):
                            for h in range(st.session_state.attention_data['numHeads']):
                                st.session_state.selected_heads.append(HeadPair(layer=l, head=h))
                    elif layer_str == ':':
                        layer = int(head_str)
                        for h in range(st.session_state.attention_data['numHeads']):
                            st.session_state.selected_heads.append(HeadPair(layer=layer, head=h))
                    elif head_str == ':':
                        head = int(layer_str)
                        for l in range(st.session_state.attention_data['numLayers']):
                            st.session_state.selected_heads.append(HeadPair(layer=l, head=head))
                    else:
                        layer = int(layer_str)
                        head = int(head_str)
                        st.session_state.selected_heads.append(HeadPair(layer=layer, head=head))
