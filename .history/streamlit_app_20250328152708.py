                ...
                with st.container():
                    ...
                    with col2:
                        if st.button("🎨", key=f"color_{group.id}", help="Change group color"):
                            st.session_state.color_change_group = group.id
                            st.experimental_rerun()

            # Handle color changes after all groups are rendered
            if st.session_state.color_change_group is not None:
                for group in st.session_state.head_groups:
                    if group.id == st.session_state.color_change_group:
